...
> 版本 `5.3-linux` · Python 3.9+ · 支持 Linux / macOS / 任何有 python3 的环境
...
# 端口被上次的实例占着？不用换端口，先停掉旧的（安全，只停本工具自己的进程）
curl -fsSL .../get.sh | sh -s -- --stop
...
`get.sh` 会先检查 `curl`/`wget` 和 `python3`，缺哪个就提示对应的安装命令，不会盲目往下跑。
...
成功的设备在结果表里会多一个**「打开」列**，点一下就在新标签页打开该设备后台，
不用再手动敲地址。地址用的是**探测时实际连通的那个**（http / https 与端口都是真实可用的一组），
所以不会出现「协议猜错打不开」的情况。

工具栏还有一个 **「打开全部成功（N）」**，一次打开所有命中设备。
注意浏览器**会拦截批量弹窗**（通常只放行第一个），被拦时用每行的「打开」逐个进即可；
工具会在状态行如实告诉你拦了几个。

> 工具**只负责打开，不代填账号密码**——打开后需要你自己登录。
> 凭据就在同一行的「命中账号密码」列里，直接复制即可。
...
# 端口被上次的实例占着：停掉它（不换端口）
python3 check.py --stop --port 5678
...
默认端口 5678。如果上一次的实例还在跑，再启动就会提示端口已被占用。**不必换端口**，一步到位：

```sh
# 停掉占用该端口的本工具实例，然后正常启动
python3 check.py --stop --port 5678
python3 check.py
```

`--stop` 的安全边界：只停「命令行里含 `check.py`」的进程，也就是本工具自己的实例。
如果该端口其实是别的服务在用，它会明确告诉你「PID xxx 是其它程序，未做任何处理」，**绝不误杀**。
不带 `--port` 时，会扫描并停掉系统里所有本工具的实例。

端口被占用时，启动提示本身也直接给出占用进程与停止命令：

```
[错误] 端口 5678 已被占用。
       占用进程  PID 12345   python3 /opt/openwrt-checker/check.py --port 5678
       停掉它    kill 12345
       停止本工具的旧实例：check.py --stop --port 5678
       或换个端口启动：--port <其它端口>
       注：--stop 只停命令行里含 check.py 的本工具实例，不动别的服务。
```

### 为什么以前会「停不掉」

一种常见情形：进程是在**非交互环境**里被拉起来的 —— 后台 `&`、管道、某些 Web 终端 / 面板 / 容器。
POSIX 规定这种场景下启动的子进程 `SIGINT` 被设为**忽略**；而 Python 只在 `SIGINT` 处置为「默认」时
才安装自己的 `KeyboardInterrupt` 处理器，于是 `Ctrl+C` 毫无反应，进程一直占着端口，看上去「用一次就不能用了」。

本版在开始监听前**显式安装** `SIGINT` / `SIGTERM` 处理器，覆盖掉继承来的「忽略」，
所以无论进程是怎么被拉起来的，`Ctrl+C` 与 `kill` 都能干净退出并释放端口。
`SIGTERM`（systemd 停止、`--stop`）走的是同一条干净退出路径，不再是被强杀。
...
- 密码不落盘、不写日志；诊断轨迹里**只记录字段名，不记录密码值**。
- 命中的凭据会显示在结果里（「命中账号密码」列），这是给你自己看的；只存在于当次任务的内存结果中，
  服务重启即消失，不写磁盘、不写日志。
- 仓库内不含任何真实凭据。
