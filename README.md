...
成功的设备在结果表里会多一个**「打开」列**，不用再手动敲地址、手动输密码。

| 情况 | 按钮 | 点下去会发生什么 |
| --- | --- | --- |
| 表单登录命中（绝大多数设备） | **打开并登录** | 新标签页打开后台，**自动用命中凭据完成登录**，落地就是已登录的后台首页 |
| 无需密码的设备 | 打开 ↗ | 新标签页直接进后台 |
| uhttpd Basic Auth 设备 | 打开 ↗ | 打开登录框（Basic 认证无法用表单代填，需手动确认一次） |

**「打开并登录」是怎么做到的？** 探测阶段本来就已经解析出了登录页的**提交地址、字段名、隐藏域**
（这也是提交凭据时用的同一套信息）。工具把这一套原样搬成一个隐藏表单，点击时用 `target="_blank"`
提交给路由器 —— 相当于浏览器替你把这个登录表单填好并点了提交，全程本地完成，不经过任何第三方。

工具栏还有一个 **「打开全部并登录（N）」**，一次打开所有命中设备。
注意浏览器**会拦截批量弹窗**（通常只放行第一个），被拦时用每行的按钮逐个进即可；工具会在状态行如实告诉你拦了几个。

> 实测记录：用真实 Chromium 跨站验证过（检查器在 `localhost`、目标在 `127.0.0.1`，属不同站）——
> 点击后新标签页正确落到 `/cgi-bin/luci/admin/status/overview`，无登录表单，即已登录。
> 这也是为什么按钮走**表单提交**而不是 `fetch`：跨站 `fetch` 带不上 Cookie，而顶层表单提交是一次真正的导航，
> 路由器下发的 `sysauth` 会话 Cookie 会被正常写入。
...
- **命中的凭据会随结果回到你自己的浏览器**——这是「打开并登录」按钮能自动填表的前提
  （本来「命中账号密码」这一列也要显示它）。凭据只存在于当次任务的内存结果里，服务重启即消失，
  且不会写入磁盘或日志。若你的浏览器/这台机器不是你独占的，请注意这一点。
- 仓库内不含任何真实凭据。
